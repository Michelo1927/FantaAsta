...
def appetibilita(df: pd.DataFrame) -> float:
    ...
    for index, row in df.iterrows():
        if int(row.iloc[-1]) > int(giocatemax):
            giocatemax = int(row.iloc[-1])
    ...
    for index, row in df.iterrows():
        ...
        if int(row.iloc[5]) > 0:
            appetibilita += float(row.iloc[7]) * int(row.iloc[5]) / 38   *20/100 #era row 2
        
        if not (
            df.columns[2].split(" ")[-1] == df.columns[6].split(" ")[-1]
            and int(row.iloc[-1]) > 5
        ):  # stesso anno
            appetibilita = (
                appetibilita * float(row.iloc[6]) * int(row.iloc[-1]) / giocatemax *80/100
            )  
        else: 
            appetibilita = float(row.iloc[7]) * int(row.iloc[5]) / 38 

        # media pesata fantamedia * convenienza rispetto alla quotazione * media scorso anno
        appetibilita=appetibilita*float(row.iloc['Punteggio'])*30/100
        if float(row.iloc[1]) == 0: pt=1
        else: pt=float(row.iloc[1])
        appetibilita = (
...
if __name__ == "__main__":
    ...
    # Write to XLSX file
    xlsx_file = "AstaPY.xlsx"
    convert_file = input("Do you want to convert the file to XLSX? (y/n): ")
    if convert_file.lower() == "y":
        df.to_excel(xlsx_file, index=False)
        print(f"File converted from {csv_file} to {xlsx_file}")
    else:
